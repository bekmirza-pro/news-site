...
import { CreateThingDto as CreateDto } from './dto/create-thing.dto';
import { UpdateThingDto as UpdateDto } from './dto/update-thing.dto';
import { ThingEntity } from './entities/thing.entity';

@Injectable()
export class ThingService {
  private readonly repository: Repository<ThingEntity>;

  constructor(@Inject(CONNECTION) connection: Connection) {
    this.repository = connection.getRepository(ThingEntity);
  }
  ...
  async create(dto: CreateDto, currentUser: CurrentUserDto) {
    try {
      let createdData = new ThingEntity();
